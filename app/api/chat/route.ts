...
const BodySchema = v.object({
  message: v.pipe(v.string(), v.trim(), v.minLength(1, "Message is empty."), v.maxLength(2000)),
});
...
export const POST = async (request: Request): Promise<Response> => {
  ...
  if (!devUserId) {
    return Response.json(
      { ok: false, error: "DEV_TELEGRAM_USER_ID is not set, so the web chat has no identity." },
      { status: 500 },
    );
  }
  ...
  if (!Number.isSafeInteger(userId)) {
    return Response.json(
      { ok: false, error: "DEV_TELEGRAM_USER_ID must be an integer." },
      { status: 500 },
    );
  }
  ...
  if (!parsed.success) {
    return Response.json(
      { ok: false, error: parsed.issues.map((issue) => issue.message).join("; ") },
      { status: 400 },
    );
  }
  ...
  const result = await handleAgentMessage({ userId, message: parsed.output.message });
